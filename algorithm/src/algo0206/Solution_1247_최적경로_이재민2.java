 ...
 * 20672KB | 1888ms
 */
public class Solution_1247_최적경로_이재민2 {
	...
	static void permu(int cnt, int sum, Pair prev) {
		if(sum > res) return ;
		
		if(cnt == n) {
			sum += Math.abs(prev.x - office.x) + Math.abs(prev.y - office.y);
		...
		for(int i=0; i<n; i++) {
			...
			visited[i] = true;
			int nSum = Math.abs(cus[i].x - prev.x) + Math.abs(cus[i].y - prev.y);
			permu(cnt + 1, sum + nSum ,cus[i]);
			visited[i] = false;
